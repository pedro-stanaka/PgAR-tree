...
DCMImage::DCMImage(string filename) {
    dcmFileFormat = NULL;
    openImage(filename);
}
...
void DCMImage::openImage(string filename) {
...
void DCMImage::saveToFile(string filename) {
...
string DCMImage::getStringTag(DCMImage::TagID tagId) const {
    /* DICOM tags have the form (gggg,eeee), thus, to compose a DcmTagKey:
     * tagId >> 16    : gets the first 16 bits (gggg) of tagId
     * tagId & 0xFFFF : gets the last 16 bits (eeee) of tagId
     */
    ...
    if (cond.bad()) {
        stringstream ss;
        ss << hex << tagId << ". DcmDataset.findAndGetString code: " << cond.code() << ".";
