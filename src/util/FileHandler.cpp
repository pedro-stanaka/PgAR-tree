...
*/
FileHandler::FileHandler(string name, int type) throw (FileException*){

    if (!this->fileExists(name))
       throw new FileException(0, "The image file cannot be opened or the file does not exists", name);
            
    switch(type){
        case 0: //FILE_IN:
            this->file.open(name.c_str(),fstream::in);
            break;
        case 1: //FILE_OUT:
            this->file.open(name.c_str(),fstream::out);
            break;
        case 2: //FILE_OUT_APP:
            this->file.open(name.c_str(),fstream::app);
            break;
        case 3: //FILE_BINARY
            this->file.open(name.c_str(),fstream::in | fstream::binary);
            break;
    }
