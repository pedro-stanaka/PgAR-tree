...
#include <gbdi/arboretum/rTree/stRNode.h>
...
class stRTree {
   public:
      ...
      */
      ...
      typedef stEuclideanMetricEvaluator < DataType > rBasicMetricEvaluator;
      ...
      */
      ...
      typedef stEuclideanMetricEvaluator < DataType > rEuclideanBasicMetricEvaluator;
      ...
      * @see void RangeQuery()
      */
      tResult * RangeQuery(basicArrayObject * sample, stDistance range);
      ...
      */
      tResult * NearestQuery(basicArrayObject * sample, stCount k, bool tie = false);
      ...
      */
      stDistance GetMbrArea(basicArrayObject *mbr) {
          stDistance mbrsize = 1.0;
          stCount dim = mbr->GetSize() / 2;
          for (stCount i = 0; i < dim; i++) {
              mbrsize *= *((DataType *)mbr->Get(dim + i)) - *((DataType *)mbr->Get(i));
          }
          return mbrsize;
      }
      ...
      */
      stDistance MinDistance(basicArrayObject *obj, basicArrayObject *mbr) {
          if (this->GetQueryMetricEvaluator() != NULL) {
              this->GetQueryMetricEvaluator()->UpdateDistanceCount();
          }
          stDistance r, sum = 0.0;
          int dim = obj->GetSize();
          for(int i = 0; i < dim; i++) {
              if (*((DataType *)obj->Get(i)) < *((DataType *)mbr->Get(i))) {
                  r = *((DataType *)mbr->Get(i));
              }
              else {
                  if (*((DataType *)obj->Get(i)) > *((DataType *)mbr->Get(dim+i)))
                      r = *((DataType *)mbr->Get(dim+i));
                  else
                      r = *((DataType *)obj->Get(i));
              }
              sum += ((*((DataType *)obj->Get(i)) - r) * (*((DataType *)obj->Get(i)) - r));
          }
          return sum;
      }
      ...
      */
      stDistance MinMaxDistance(basicArrayObject *obj, basicArrayObject *mbr) {
          ...
          stDistance rmk, rMi;
          for(int i = 0; i < dim; i++) {
              rMi = (*((DataType *)obj->Get(i)) >= (*((DataType *)mbr->Get(i)) + *((DataType *)mbr->Get(i+dim)))/2) ? *((DataType *)mbr->Get(i)) : *((DataType *)mbr->Get(i+dim));
              S += pow( *((DataType *)obj->Get(i)) - rMi, 2 );
          }
          for(int k = 0; k < dim; k++) {
              rmk = ( *((DataType *)obj->Get(k)) <=  (*((DataType *)mbr->Get(k)) + *((DataType *)mbr->Get(k+dim))) / 2 ) ? *((DataType *)mbr->Get(k)) : *((DataType *)mbr->Get(k+dim));
              sum = pow(*((DataType *)obj->Get(k)) - rmk, 2 );
              rMi = (*((DataType *)obj->Get(k)) >= (*((DataType *)mbr->Get(k)) + *((DataType *)mbr->Get(k+dim))) / 2 ) ? *((DataType *)mbr->Get(k)) : *((DataType *)mbr->Get(k+dim));
              sum += S - pow(*((DataType *)obj->Get(k)) - rMi, 2);
              if (sum < minimum)
                  minimum = sum;
          }
          return minimum;
      ...
      * point
      */
      stDistance MaxDistance(basicArrayObject *obj, basicArrayObject *mbr) {
          stDistance sum = 0.0;
          stDistance maxdiff;
          int dim = obj->GetSize();
          for(int i = 0; i < dim; i++) {
              maxdiff = fabs(*((DataType *)obj->Get(i)) - *((DataType *)mbr->Get(i)));
              if (maxdiff < fabs(*((DataType *)obj->Get(i)) - *((DataType *)mbr->Get(i+dim))))
                  maxdiff = fabs(*((DataType *)obj->Get(i)) - *((DataType *)mbr->Get(i+dim)));
              sum += maxdiff * maxdiff;
          }
          return sum;
      }
   ...
   private:
      ...
      */
      int InsertRecursive(stPageID currNodeID, basicArrayObject * newObj, basicArrayObject * mbrObj, stSubtreeInfo & promo1, stSubtreeInfo & promo2);
      ...
      */
      void SplitIndex(stRIndexNode * oldNode, stRIndexNode * newNode, basicArrayObject * newMbr, stPageID newNodeID, stCount newNEntries, bool sonIsLeaf);
      ...
      */
      void AddNewRoot(basicArrayObject *mbrA, stPageID nodeID1, stCount nEntries1, bool sonIsLeaf1,
                      basicArrayObject *mbrB, stPageID nodeID2, stCount nEntries2, bool sonIsLeaf2);
      ...
      */
      basicArrayObject * GetIndexMbr(stRIndexNode *node) {
          ...
          stCount dimensionality = tmp.GetSize() / 2; // tmp is already a mbr
          basicArrayObject *mbr = new basicArrayObject(dimensionality * 2);
          stDistance d, mind, maxd;
          for (stCount j = 0; j < dimensionality; j++) {
              mind = MAXDOUBLE;
              maxd = -MAXDOUBLE;
              for (stCount i = 0; i < node->GetNumberOfEntries(); i++) {
                  tmp.Unserialize(node->GetObject(i), node->GetObjectSize(i));
                  if (*((DataType *)tmp.Get(j)) < mind)
                      mind = *((DataType *)tmp.Get(j));
                  if (*((DataType *)tmp.Get(j)) > maxd)
                      maxd = *((DataType *)tmp.Get(j));

                  if (*((DataType *)tmp.Get(dimensionality+j)) < mind)
                      mind = *((DataType *)tmp.Get(dimensionality+j));
                  if (*((DataType *)tmp.Get(dimensionality+j)) > maxd)
                      maxd = *((DataType *)tmp.Get(dimensionality+j));
              }
              mbr->Set(j,mind);
      ...
      */
      basicArrayObject * GetLeafMbr(stRLeafNode *node) {
          basicArrayObject tmp;
          tmp.Unserialize(node->GetObject(0), node->GetObjectSize(0));
          stCount dimensionality = tmp.GetSize();
          basicArrayObject *mbr = new basicArrayObject(dimensionality * 2);
          stDistance d, mind, maxd;
          for (stCount j = 0; j < dimensionality; j++) {
              mind = MAXDOUBLE;
      ...
      */
      void EnlargeMbr(basicArrayObject *mbr, basicArrayObject *obj) {
          stCount dimensionality = obj->GetSize();
          for (stCount i = 0; i < dimensionality; i++) {
              if (*((DataType *)obj->Get(i)) < *((DataType *)mbr->Get(i)))
                  mbr->Set(i, *((DataType *)obj->Get(i)));
              if (*((DataType *)obj->Get(i)) > *((DataType *)mbr->Get(dimensionality+i)))
                  mbr->Set(dimensionality+i, *((DataType *)obj->Get(i)));
          }
      }
      ...
      */
      basicArrayObject *GetUpperMbrObject(basicArrayObject *mbr) {
          ...
          for (int i = 0; i < dim; i++) {
              obj->Set(i,*((DataType *)mbr->Get(dim+i)));
          }
          return obj;
      }
      ...
      */
      stDistance Overlap(basicArrayObject *mbr1, basicArrayObject *mbr2) {
          stDistance sum = 1.0;
          int dimension = mbr1->GetSize() / 2;
          int r1pos = 0, r2pos = 0, r1last = dimension;
          stDistance r1_lb, r1_ub, r2_lb, r2_ub;
          ...
          while (r1pos < r1last) {
              r1_lb = *((DataType *)LowerMbr1->Get(r1pos));
              r1_ub = *((DataType *)UpperMbr1->Get(r1pos));
              r2_lb = *((DataType *)LowerMbr2->Get(r2pos));
              r2_ub = *((DataType *)UpperMbr2->Get(r2pos));
              r1pos++;
              r2pos++;
      ...
      */
      stDistance GetDistanceBetweenMbrs(basicArrayObject *mbr1, basicArrayObject *mbr2) {
          ...
          stDistance d1, d2, d3, d4, sum = 0.0;
          int i, dim = mbr1a->GetSize();
          // get the sum of the smallest difference for each dimension
          for (i = 0; i < dim; i++) {
               d1 = fabs(*((DataType *)mbr1a->Get(i)) - *((DataType *)mbr2a->Get(i)));
               d2 = fabs(*((DataType *)mbr1a->Get(i)) - *((DataType *)mbr2b->Get(i)));
               d3 = fabs(*((DataType *)mbr1b->Get(i)) - *((DataType *)mbr2a->Get(i)));
               d4 = fabs(*((DataType *)mbr1b->Get(i)) - *((DataType *)mbr2b->Get(i)));
      ...
      */
      bool IntersectionBetweenMbrAndBall(basicArrayObject *mbr, basicArrayObject *ballcenter, stDistance radius) {
      ...
      */
      void RangeQuery(stPageID pageID, tResult * result, basicArrayObject * sample, stDistance range);
      ...
      */
      void NearestQuery(tResult * result, basicArrayObject * sample, stDistance rangeK, stCount k);
