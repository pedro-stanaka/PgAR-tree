...
int stRTree<DataType,OIDType>::InsertRecursive(
        stPageID currNodeID, basicArrayObject * newObj, basicArrayObject * mbrObj, stSubtreeInfo & promo1, stSubtreeInfo & promo2){
    ...
    if (currNode->GetNodeType() == stRNode::INDEX){
        ...
        stDistance tmpwidth = this->GetMbrArea(tmpmbr);
        delete tmpmbr;
        ...
        if (insertIdx >= 0) { // Don't split!
            ...
            stDistance newtmpwidth = this->GetMbrArea(promo1.Mbr);
...
int stRTree<DataType,OIDType>::ChooseSubTree(stPage * currPage, basicArrayObject * obj) {
    ...
    */
    ...
    if (insideCount == 1) {
        ...
        stPageID tmpRootID = rIndexNode->GetIndexEntry(minIndex).PageID;
        stCount tmpNObjects = rIndexNode->GetIndexEntry(minIndex).NEntries;
...
//------------------------------------------------------------------------------
template <class DataType, class OIDType>
void stRTree<DataType,OIDType>::SplitIndex(stRIndexNode * oldNode, stRIndexNode * newNode, basicArrayObject * newMbr, stPageID newNodeID, stCount newNEntries, bool sonIsLeaf) {
    basicArrayObject *mbr;
    stCount i, j, k, position1, position2, idx, numberOfEntries = oldNode->GetNumberOfEntries();
    stDistance d, mindist, maxdist;
    ...
    allPageIDs = new stPageID[numberOfEntries];
    allNEntries = new stCount[numberOfEntries];
    ...
    case stRTree::smEXPONENTIAL:
         ...
         * enlargement.
         */
        stDistance totalMbrEnlargement = MAXDOUBLE;
...
void stRTree<DataType,OIDType>::SplitLeaf(stRLeafNode * oldNode, stRLeafNode * newNode, basicArrayObject * newObj,
    ...
    stDistance d, mindist, maxdist;
    stCount position1, position2, i, j, k, numberOfEntries;
...
//------------------------------------------------------------------------------
template <class DataType, class OIDType>
void stRTree<DataType,OIDType>::AddNewRoot(basicArrayObject *mbrA, stPageID nodeID1, stCount nEntries1, bool sonIsLeaf1,
...
//------------------------------------------------------------------------------
template <class DataType, class OIDType>
void stRTree<DataType,OIDType>::NearestQuery(tResult * result, basicArrayObject * sample, stDistance rangeK, stCount k) {
...
//------------------------------------------------------------------------------
template <class DataType, class OIDType>
void stRTree<DataType,OIDType>::RangeQuery(stPageID pageID, tResult * result, basicArrayObject * sample, stDistance range) {
    stPage * currPage;
    stRNode * currNode;
    basicArrayObject tmpObj;
    stDistance distance;
