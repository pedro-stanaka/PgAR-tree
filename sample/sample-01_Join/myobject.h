...
//
// This file implements the 2 classes required by the ubTree Library User
// Layer.
//
...
// interface to manipulate its information and also implements the ubObject
// interface.
//
// TCityDistanceEvaluator implements the ubMetricEvaluator interface. It will
// mesure the distance between 2 TCity instances.
//
// With these classes, it is possible to define and instantiate any metric tree
// defined by the ubTree Library.
//
// Author: Author: Fabio
...
*
* @version 1.0
* @author Author: Marcos Rodrigues Vieira (mrvieira@icmc.sc.usp.br)
*/
   ...
   public:
      ...
      */
      ...
      stDistance GetDistance2(TCity *obj1, TCity *obj2){
         double lat1, lat2, long1, long2, parcidist, finaldist;

         lat1 = obj1->GetLatitude();
         lat2 = obj2->GetLatitude();
         long1 = obj1->GetLongitude();
         long2 = obj2->GetLongitude();
         parcidist = acos(sin(lat1*M_PI/180)*sin(lat2*M_PI/180)+cos(lat1*M_PI/180)*cos(lat2*M_PI/180)*cos(long2*M_PI/180-long1*M_PI/180));
         //angulo em radianos
         finaldist = 6370 * parcidist;
         return (finaldist*finaldist);
        }
