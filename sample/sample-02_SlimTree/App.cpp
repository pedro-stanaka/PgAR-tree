   ...
   if (SlimTree != NULL){
      ...
      if (!in.is_open()){
         ...
         while(in.getline(cityName, 200, '\t')){
            ...
            contObj++;
            name = cityName;
            LB->Items->Add(name + " (" + FloatToStr(dLat) + ") [" + FloatToStr(dLong) + "]");
         }//end while
         LB->Sorted = true;
         // Build the message.
         messageText = "Added ";
...
void __fastcall TForm1::BRunClick(TObject *Sender)
{
   ...
   if (!result->GetNumOfEntries()){
      ...
      for (myResult::tItePairs ite = result->beginPairs(); ite != result->endPairs(); ite++) {
         tmp = (TCity *)(*ite)->GetObject();
         text = " Distance = ";
         text += (*ite)->GetDistance();
         text += " - Object: ";
      ...
      for (int i = 0; i < result->GetNumOfEntries(); i++) {
         tmp = (TCity *)result->GetPair(i)->GetObject();
         text = " Distance = ";
         text += result->GetPair(i)->GetDistance();
         text += " - Object: ";
         text += tmp->GetName().c_str();
         text += "; ";
         text += tmp->GetLatitude();
...
void __fastcall TForm1::LBClick(TObject *Sender) {
    AnsiString item = LB->Items->Strings[LB->ItemIndex];
    ECityName->Text = item.SubString(1,item.Pos("(")-2);
    ECityLatitude->Text = item.SubString(item.Pos("(") + 1, item.Pos(")") - item.Pos("(") - 1);
    ECityLongitude->Text = item.SubString(item.Pos("[") + 1, item.Pos("]") - item.Pos("[") - 1);
}
//---------------------------------------------------------------------------
